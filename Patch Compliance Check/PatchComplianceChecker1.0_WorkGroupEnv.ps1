...
#You can mention IPs of the workgroup servers under computers.txt. If you are using IP 
...
$connection1 = Test-NetConnection -ComputerName $svr -Port 5985
$connection2 = Test-NetConnection -ComputerName $svr -Port 5986

if ((test-Connection -ComputerName $svr -Count 2 -Quiet) -or ($connection1.TcpTestSucceeded) -or ($connection2.TcpTestSucceeded)) 
{
 
    $hostname = Invoke-Command -Computername $svr -Credential $Credential -UseSSL -Scriptblock {(get-WmiObject win32_computersystem).Name}
    ...
    $hotfixeid = (Get-WmiObject -Class Win32_QuickFixEngineering  | Select-Object HotFixID | Sort-Object InstalledOn -Descending | Select-Object -First 1).hotfixid
    $installedon = (Get-WmiObject -Class Win32_QuickFixEngineering | Select-Object InstalledOn | Sort-Object InstalledOn -Descending | Select-Object -First 1).Installedon
...
if (test-Connection -ComputerName $svr -Count 2 -Quiet)
 ...
 $installedon = "NA"
 $hotfixCount = "NA"
 $ping = $svr + "is not pinging nor responding to connections over 5985 or 5986. The server is either poweredoff state OR ICMP is disabled and WinRM is not configured"
 $PatchCompliance = "NA"
}
